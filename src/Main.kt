...
fun agoToText(secondsA: Int): String {
    return when (secondsA) {
        ...
        in (60 * 60) until (24 * 60 * 60) -> hoursAgo((secondsA / (60 * 60))) // Теперь правильно работает
...
fun zadanie2() {
    ...
    when(Card.lowercase()) {
        "vkpay" -> {
            ...
            if(MonthAmount <= 40000 && SummAmount <= 15000) println("Реквизит ${Card} итоговая сумма перевода ${SummAmount}")
        ...
        "mastercard", "maestro" -> {
            if(MonthAmount <= 600000 && SummAmount <= 150000) {
                if(SummAmount < 75000) println("Реквизит ${Card} итоговая сумма перевода ${SummAmount}")
